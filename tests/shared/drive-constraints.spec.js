...
describe('Shared: DriveConstraints', function() {
  ...
  describe('.getDriveImageCompatibilityStatuses', function() {
    ...
    const expectStatusTypesAndMessagesToBe = (resultList, expectedTuples) => {
      ...
      const expectedTypesSorted = _.sortBy(_.map(expectedTuples, (tuple) => {
        return constraints.COMPATIBILITY_STATUS_TYPES[tuple[0]];
      }));
      const expectedMessagesSorted = _.sortBy(_.map(expectedTuples, (tuple) => {
        return constraints.COMPATIBILITY_STATUS_MESSAGES[tuple[1]];
      }));
      const resultTypesSorted = _.sortBy(_.map(resultList, 'type'));
      const resultMessagesSorted = _.sortBy(_.map(resultList, 'message'));

      console.log(_.map(resultList, (tuple) => {
        return [ tuple.type, tuple.message ];
      }));
      console.log(`${Array.from(resultTypesSorted)} = ${Array.from(expectedTypesSorted)}`);
      console.log(`${Array.from(resultMessagesSorted)} = ${Array.from(expectedMessagesSorted)}`);

      m.chai.expect(resultTypesSorted).to.deep.equal(expectedTypesSorted);
      m.chai.expect(resultMessagesSorted).to.deep.equal(expectedMessagesSorted);
    };
